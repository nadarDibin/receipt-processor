...
class ReceiptProcessor:
    ...
    AMOUNT_PATTERNS = [
        r'₹\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)',      # ₹236.28, ₹7202.0 or ₹1,234.56
        r'Rs\.?\s*(\d+(?:,\d{3})*\.\d{1,2})',       # Rs 236.28 or Rs.236.28 (require decimal)
        r'INR\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)',     # INR 236.28
        r'\$\s*(\d+(?:,\d{3})*(?:\.\d{1,2})?)',      # $236.28
        r'(?<![a-zA-Z₹$])(\d+(?:,\d{3})*\.\d{1,2})(?![a-zA-Z])',  # Just 236.28, 7202.0 (not preceded by letters or currency)
        r'(?<![a-zA-Z₹$])(\d+(?:,\d{3})+)(?![.\d])',             # Whole numbers with commas like 7,042 (not followed by decimal)
    ]
    ...
    def extract_amount_from_line(self, line: str) -> List[float]:
        ...
        for pattern in self.AMOUNT_PATTERNS:
            for match in re.finditer(pattern, line):
                ...
                start, end = match.span(1)  # Get span of the captured group
                overlap = any(start < pos[1] and end > pos[0] for pos in found_positions)
                if overlap:
                    continue
